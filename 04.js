(() => {
  ...
  (function animate() {
    ofx.fillStyle = `rgba(122, 28, 194, ${(R() * 0.06 - 0.001) + 0.001})`;
    ...
    nodes.forEach((n, idx) => {
      ...
      if (leader) {
        ...
        if (n.delayBuffer.length > 130) {
          err = n.delayBuffer.shift();
        }

        n.setpoint += n.gain * err;
      } else {
        n.setpoint += (R() - 0.5) * 25 * Math.PI;
      }
